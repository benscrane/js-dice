...
function updateThreshold(game) {
    ...
    term.moveTo(1,5);
    if (game.lowest > 0) {
        term(`${game.lowest} in ${game.numRolls} roll${game.numRolls == 1 ? "" : "s"}`);
    }
}
...
exports.getRollDecision = async function() {
    term.moveTo(1, term.height);
    term.eraseLine();
    term(centerLine('Press ENTER to roll; S to stay'));
...
exports.getContinueDecision = async function() {
    term.moveTo(1, term.height);
    term.eraseLine();
    term(centerLine('Press ENTER to continue; Q to quit'));
    let termPromise = await term.yesOrNo({
        yes: ['ENTER'],
        no: ['q', 'Q']
    }).promise;
...
function updateRollDisplay(game, rolledDice, roll) {
    term.saveCursor();
...
function clearRollDisplay() {
    term.eraseArea(1, 10, term.width, 3);
}

exports.clearRollDisplay = clearRollDisplay;
exports.updateRollDisplay = updateRollDisplay;